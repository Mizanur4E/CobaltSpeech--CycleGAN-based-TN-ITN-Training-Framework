'''
Simple GAN, generator takes no noise, and discriminator takes generator input as context.
used SGD optimizer with learning rate 0.0005 and momentum =0.9 and nesterov= True
same optimizer for both discriminator and generator
'''
...
class textNoiseCGAN():
    ...
    def __init__(self):
        ...
        self.text2vecNet = keras.models.load_model('text2vecNet')
        ...
        one_enc_text= self.generator(GAN_text)
        self.discriminator.trainable = False
        self.text2vecNet.trainable = False
        output_text= self.text2vecNet(one_enc_text)
        #print(output_text.shape)
        label= self.discriminator([output_text,GAN_text])
        self.GAN = Model(GAN_text,label)
        GAN_optimizer = SGD(0.0005,momentum= 0.9,nesterov= True)
    ...
    def build_generator(self):
        ...
        #merged = layers.concatenate([text,noise])
        en_out= self.Bi_en(text)
        de_in = self.Rep_vect(en_out)
        de_out= self.Bi_de(de_in)
        final_op =  layers.TimeDistributed(layers.Dense(self.vocab_size,activation='softmax'))(de_out)
    ...
    def build_discriminator(self):
        ...
        concatenated= layers.concatenate([text,context],axis=1)
        ...
        conv2_op=layers.Dropout(0.4)(conv2_op)
    ...
    def train(self,X_nnrmlzd, X_nrmlzd,epochs,batch_size= 128,sample_interval = 100):
        ...
        for epoch in range(epochs):
            ...
            gen_nrmlzd = self.text2vecNet.predict(gen_nrmlzd)
